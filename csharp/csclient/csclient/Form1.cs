...
namespace csclient
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            InitializeComponent();
            clients = new Clients();
            clients.Connected += ChatClient_Connected;
            clients.Disconnected += ChatClient_Disconnected;
            clients.MessageReceived += ChatClient_MessageReceived;
        }
        ...
        private async Task SendChatMessage()
        {
            ...
            if (!string.IsNullOrWhiteSpace(message))
            {
                ...
                {
                    await clients.SendMessageAsync(message);
                }
                else {
                    username =  message+"";
                    AppendLogMessage($"채팅 서버에 연결 중... 사용자 이름: {username}", System.Drawing.Color.Gray);
