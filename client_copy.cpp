...
int main() {
    ...
    while (true) {
        ...
        if(nickname.empty()){
            std::cout << "사용자 이름 입력: ";
            std::getline(std::cin, nickname); // 닉네임 입력 받기
            bytes_sent = send(client_sock, nickname.c_str(), nickname.length(), MSG_NOSIGNAL);
        }else{
            std::cout << nickname + ": ";
            std::getline(std::cin, message); // 사용자 입력 받기
            bytes_sent = send(client_sock, message.c_str(), message.length(), MSG_NOSIGNAL);
        }
